...
public class mod_MLhook extends BaseMod{
	...
	public void generateConsoleOutput(){
		...
		for (int n=0;n<itemlist.length;n++){
			if (itemlist[n]!=null){
				String output=magicstring+"<>"+type+"<>"+itemlist[n].itemID;
		...
		TreeMap<Object,String> idmap=new TreeMap<Object,String>();
		...
		while(it.hasNext()){
			Object id = it.next();
			System.out.println(magicstring+"<>"+type+"<>"+id+"<>"+idmap.get(id));
		}
