...
public class Encoder {
    ...
    class LiteralEncoder {
        class Encoder2 {
            ...
            public void encodeMatched(lzma.sdk.rangecoder.Encoder rangeEncoder,
            ...
            public int getPrice(boolean matchMode, byte matchByte, byte symbol) {
                ...
                if (matchMode) {
                    for (; i >= 0; i--) {
                        int matchBit = (matchByte >> i) & 1;
                        int bit = (symbol >> i) & 1;
                        price += lzma.sdk.rangecoder.Encoder.getPrice(
                                m_Encoders[((1 + matchBit) << 8) + context],
                                bit);
                        context = (context << 1) | bit;
                        if (matchBit != bit) {
                            i--;
    ...
    int getPureRepPrice(int repIndex, int state, int posState) {
        ...
        if (repIndex == 0) {
            ...
            price += lzma.sdk.rangecoder.Encoder
            ...
            if (repIndex == 1) {
                ...
                price += lzma.sdk.rangecoder.Encoder.getPrice(_isRepG2[state],
                        repIndex - 2);
            }
        }
        return price;
    ...
    int getOptimum(int position) throws IOException {
        if (_optimumEndIndex != _optimumCurrentIndex) {
            int lenRes = _optimum[_optimumCurrentIndex].PosPrev
        ...
        _optimum[1].Price = lzma.sdk.rangecoder.Encoder
        ...
        int matchPrice = lzma.sdk.rangecoder.Encoder
                .getPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax)
                        + posState]);
        int repMatchPrice = matchPrice
                + lzma.sdk.rangecoder.Encoder.getPrice1(_isRep[_state]);

        if (matchByte == currentByte) {
            int shortRepPrice = repMatchPrice
        ...
        int normalMatchPrice = matchPrice
                + lzma.sdk.rangecoder.Encoder.getPrice0(_isRep[_state]);
        ...
        while (true) {
            ...
            repMatchPrice = matchPrice
                    + lzma.sdk.rangecoder.Encoder.getPrice1(_isRep[state]);
            ...
            if (!nextIsChar && matchByte != currentByte) {
                ...
                if (lenTest2 >= 2) {
                                    ...
                                    .getPrice1(_isRep[state2]);
                    {
                        ...
                        int curAndLenPrice = nextRepMatchPrice
            ...
            if (newLen >= startLen) {
                normalMatchPrice = matchPrice
                        + lzma.sdk.rangecoder.Encoder.getPrice0(_isRep[state]);
                ...
                for (int lenTest = startLen;; lenTest++) {
                    ...
                    if (lenTest == _matchDistances[offs]) {
                        if (lenTest < numAvailableBytesFull) {
                            int t = Math.min(numAvailableBytesFull - 1
        ...
        while (true) {
            ...
            if (len == 1 && pos == -1) {
                ...
                if (!Base.stateIsCharState(_state)) {
                    byte matchByte = _matchFinder.getIndexByte(0
                            - _repDistances[0] - 1 - _additionalOffset);
